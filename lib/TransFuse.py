...
class TransFuse_S(nn.Module):
    def __init__(self, num_classes=1, drop_rate=0.2, normal_init=True, pretrained=False):
        ...
        self.net = HarDMSEG()
        if pretrained:
            weights = torch.load('D:/Wyy_FuTransHNet/HarD-MSEG-best.pth', map_location="cpu")  # 加载到 CPU
            self.net.load_state_dict(weights)  # 加载权重
            if torch.cuda.is_available():
                self.net = self.net.cuda()  # 移动到 GPU
        ...
        self.final_x = nn.Sequential(
            Conv(64, 64, 1, bn=True, relu=True),
            Conv(64, 64, 3, bn=True, relu=True),
            Conv(64, num_classes, 3, bn=False, relu=False)
        )

        self.final_1 = nn.Sequential(
            Conv(64, 64, 3, bn=True, relu=True),              #nn.relu()：将小于0的置为0，大于0的不变   0为黑色，1为白色
            Conv(64, num_classes, 3, bn=False, relu=False)    #因为已经变为单通道了，二值图像，不需要在进行nn.relu()激活函数了
        )
    ...
    def forward(self, imgs, labels=None):
        ...
        # t_0，t_1，t_2进行特征融合
        # T_x_b = self.Translayer2_0(x_b)
        #
        #
        # T_x_b_1 = self.Translayer2_1(x_b_1)
        #
        # T_x_b_2 = self.Translayer3_1(x_b_2)
        #
        #
        #
        # T = self.CFM(T_x_b, T_x_b_1, T_x_b_2)
        ...
        # decoder part
        map_x = F.interpolate(self.final_x(x_u_2), scale_factor=4, mode='bilinear')  # CNN融合后的预测值  [16,256,22,22]
        ...
        return map_x, map_1, map_2    #[20,1,352,352]
...
class Conv(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.bn is not None:
            x = self.bn(x)
        if self.relu is not None:
            x = self.relu(x)       #激活函数，如果输入大于0，直接返回输入提供的值，若<=0,则返回0
        return x
